...
def skill_view(
    ...
    try:
        ...
        reference_index = []
        if skill_dir and linked_files.get("references"):
            from agent.skill_utils import extract_support_file_description

            for rel in linked_files["references"]:
                entry = {"path": rel}
        ...
        result = {
            ...
            "usage_hint": "To view linked files, call skill_view(name, file_path) where file_path is e.g. 'references/api.md' or 'assets/config.yaml'. reference_index describes when to load each reference — load only what the task needs." if linked_files else None,
            **readiness,
