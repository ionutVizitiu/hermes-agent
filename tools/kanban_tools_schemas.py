...
KANBAN_ASSIGN_SCHEMA = _schema(
    ...
    {
        "task_id": _prop("string", "Task id to (re)assign."),
        "profile": _prop(
            "string",
            "Target profile name (must match an existing profile, e.g. "
            "'worker', 'wp-builder'), or 'none' to unassign.",
        ),
    },
    ["task_id", "profile"],
)
