...
from tools.kanban_tools_schemas import (
    KANBAN_ASSIGN_SCHEMA, KANBAN_ATTACH_SCHEMA,
...
def _handle_assign(args: dict, **kw) -> str:
    ...
    (claimed) and resets the failure streak on a profile change. A
    RuntimeError from a claimed task is surfaced as-is so the orchestrator
    knows to wait or ask a human to ``hermes kanban reclaim``.
    """
    _reject_delegated_child_mutation("kanban_assign")
    _require_orchestrator_tool("kanban_assign")
    tid = args.get("task_id")
    _check(tid, "task_id is required")
    _check(
        "profile" in args,
        "profile is required — the profile name to assign, or 'none' to unassign")
    profile = _normalize_profile(args.get("profile"))
    with _board(args.get("board")) as (kb, conn):
        _check(kb.assign_task(conn, str(tid), profile), f"no such task: {tid}")
...
_ORCHESTRATOR_TOOLS = frozenset({"kanban_list", "kanban_unblock", "kanban_assign"})
_TOOLS = (
    ...
    ("kanban_assign", KANBAN_ASSIGN_SCHEMA, _handle_assign, "🎯"),
