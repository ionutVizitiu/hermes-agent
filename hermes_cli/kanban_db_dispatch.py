...
def _resolve_hermes_argv() -> list[str]:
    ...
    same-directory file), then ``sys.executable -m hermes_cli.main``, and only
    as a last resort ``which("hermes")`` (Windows: safe PATH search, batch
    shims fall back to the module form).

    The module form comes before the PATH shim deliberately: worker launches
    must stay bound to the exact interpreter/import path already running the
    dispatcher, while PATH shims are allowed to point at alternate entrypoints
    such as TUI wrappers, stale venv scripts, or platform-specific shebangs.

    Kept local (not imported from ``gateway.run``) because ``hermes_cli`` sits
    below ``gateway`` in the dependency order.
    """
    import shutil
