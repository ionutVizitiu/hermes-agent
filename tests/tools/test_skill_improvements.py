...
class TestSkillCreationDir:
    ...
    def test_created_skill_in_creation_dir_is_writable(self):
        ...
        patch_result = _patch_skill(
            "custom-created", "Step 1: Do the thing.", "Step 1: Updated."
        )
        write_result = _write_file(
            "custom-created", "references/example.md", "# Example\n"
        )

        assert patch_result["success"] is True
        assert write_result["success"] is True
        assert "Step 1: Updated." in (
            self.custom_dir / "custom-created" / "SKILL.md"
        ).read_text()
        assert (
            self.custom_dir / "custom-created" / "references/example.md"
        ).exists()
